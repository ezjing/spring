...
public class BoardController {
    // RESTful이란 : 자원의 이름으로 구분하여 해당 자원의 정보를 주고 받는 것(주소는 같고 사용하는 메소드가 다름. 메소드로 구분함)
    // HTTP URI를 통해 자원을 명시하고 HTTP Method(GET, POST, PUT, DELETE)를 통해 자원에 대한 CRUD 명령을 적용하는 것

    // URI : Uniform Resource Identifier 의 줄임말, 자원(서버주소) 식별자
    // URL : Uniform Resource Locator 의 줄임말, 자원의 위치
    // 현재는 거의 같은 의미로 사용함

    // RESTfull 방식은 데이터를 전송하는 방법을 지정함(동일한 주소를 가져도 메소드에 따라 다르게 작동)
    // HttpMethod를 사용하여 GET(조회), POST(입력), PUT(수정), DELETE(삭제)로 구분하여 사용함
    // @RequestMapping 어노테이션 사용 시 URI 를 value 속성을 사용하여 설정하고, method 설정을 사용하여 GET, POST, PUT, DELETE를 설정
    // @GetMapping, @PostMapping, @PutMapping, @DeleteMapping 을 사용하여 구분해도 됨

    // 전체 URL 생성 방법 :
    // 서버URL:포트번호/기본 주소/추가 주소 or 게시물 번호/게시물 번호

    // index 사용 하지 않음 boardList를 메인 페이지로 주소를 간편하게 주려고 주석처리함
...
//    }
    ...
    public ModelAndView boardDetail(@PathVariable("boardIdx") int boardIdx) throws Exception {  // 데이터의 이동 : {boardIdx} -> "boardIdx" -> int boardIdx
    ...
    // 게시물 등록, 사용자 입력 페이지
    @RequestMapping(value = "/board3/insert", method = RequestMethod.GET)   //조회는 GET
    ...
    // 게시물 등록, 내부 프로세스
    @RequestMapping(value = "/board3/insert", method = RequestMethod.POST)  // 입력은 POST
    ...
    // 게시물 수정
    @RequestMapping(value = "/board3/{boardIdx}", method = RequestMethod.PUT)   // 수정은 PUT
    ...
    // 게시물 삭제
    @RequestMapping(value = "/board3/{boardIdx}", method = RequestMethod.DELETE)    // 삭제는 DELETE
