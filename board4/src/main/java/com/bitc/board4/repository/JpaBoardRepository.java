...
public interface JpaBoardRepository extends JpaRepository<JpaBoardEntity, Integer> {    // <사용하고자하는 데이터베이스 엔티티, pk의  데이터타입>
    // 메소드 쿼리 : CrudRepository에서 지원하는 기본 쿼리 명령 메소드를 가지고 원하는 모든 기능을 사용할 수 없기 때문에 특정 키워드를 사용하여 사용자 정의 메소드를 생성하는 방식의 메소드
    // 메소드 쿼리를 사용하여 필요한 명령을 전달함
    List<JpaBoardEntity> findAllByOrderByBoardIdxDesc();    // 명령어 + By + 명령어1 + By + 명령어2 + By + ...
